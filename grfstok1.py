...
def fetch_stock_advanced_engine(ticker_str):
    ...
    try:
        ...
        # שליפת נתוני פונדמנטלס מורחבים מה-API (שיפור)
        ...
        if div_yield: div_yield = div_yield * 100  # המרה לאחוזים
        gross_margins = info.get('grossMargins', 0.0)
        if gross_margins: gross_margins = gross_margins * 100  # המרה לאחוזים
        ...
        return {
            ...
            "dividend_yield": div_yield, "gross_margins": gross_margins,
        ...
        if "last_context" in st.session_state and st.session_state.last_context["ticker"] == target_ticker:
            ...
            if st.button("✨ הפק דוח השקעות מורחב ומלא של ה-AI", type="primary"):
                with st.spinner("מנוע ה-AI מפיק דוח עומק מפורט..."):
                    ...
                    נתוני שוק: סקטור {ctx['data']['sector']}, בטא {ctx['data']['beta']:.2f}, מכפיל נוכחי {ctx['data']['pe']}, מכפיל עתידי {ctx['data']['forward_pe']}, תשואת דיבידנד {ctx['data']['dividend_yield']:.2f}%, שולי רווח גולמי {ctx['data']['gross_margins']:.1f}%.
                    תנודתיות שנתית: {ctx['data']['volatility']:.1f}%. קונזנזוס אנליסטים: {ctx['data']['analyst_rating']}, מחיר יעד ${ctx['data']['target_price']}.
                    ...
                    for attempt in range(3):
                        try:
                            ...
                            response_full = client.models.generate_content(
                                model='gemini-2.5-flash', contents=user_context, 
                    ...
                    if response_text:
                        ...
                        # שדרוג: כפתור הורדה ייעודי של הדוח לקובץ טקסט
                        st.download_button(
                    ...
                    for t in tickers:
                        ...
                        if d:
                            ...
                            comparison_data.append({
                                ...
                                "P/E נוכחי": d["pe"], "P/E עתידי": d["forward_pe"], 
                                "תשואת דיבידנד": f"{d['dividend_yield']:.2f}%" if d['dividend_yield'] != 'N/A' else '0.0%',
