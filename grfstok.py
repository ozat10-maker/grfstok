...
def analyze_ticker(df, info, investment_amount, risk_percent, ticker_name, portfolio_total_value, current_holding_value, selected_risk_profile):
    """מנוע הניתוח הטכני, הניקוד וניהול הסיכונים המשודרג לרמת סיכון משתמש"""
    ...
    score = 0
    if current_price > ma200_curr: score += 1
    else: score -= 1

    if current_rsi > 70: score -= 2
    elif current_rsi < 30: score += 2

    # שילוב שיקולי תיק: הגנת חשיפת יתר
    ...
    # הגדרת סף הניקוד ומרחק הסטופ-לוס לפי פרופיל הסיכון הנבחר
    if "סולידי" in selected_risk_profile:
        buy_threshold = 2       # דורש ניקוד גבוה ושמרני כדי לקנות
        sl_multiplier = 0.98    # סטופ לוס קרוב והדוק מאוד (2% מתחת לתמיכה)
    elif "אגרסיבי" in selected_risk_profile:
        buy_threshold = 0       # פחות מחמיר, מאפשר כניסה גם במצבים תנודתיים
        sl_multiplier = 0.94    # סטופ לוס רחב שנותן מקום לנשימה (6% מתחת לתמיכה)
    else: # מאוזן
        buy_threshold = 1
        sl_multiplier = 0.96    # סטופ לוס סטנדרטי (4% מתחת לתמיכה)
    ...
    # החלת המכפיל המותאם אישית של הסטופ לוס
...
if not ticker_1:
    ...
    if df1.empty or len(df1) < 200:
        ...
        # שליחת רמת הסיכון הנבחרת למנוע השיקולים
        ...
        # הצגת רמת הסיכון שנלקחה בחשבון בדוח
        st.write(f"ℹ️ הדוח מותאם אישית עבור פרופיל משקיע: **{risk_profile}**.")

        # --- ב. תצוגת מחשבון ניהול הסיכונים ---
        st.markdown("---")
        ...
        def show_ui_metrics(res, ticker_name):
            ...
            if res['verdict_type'] == "WAIT":
                ...
                st.markdown(f"""
                * **אין לבצע קנייה במחיר השוק הנוכחי (${res['current_price']:.2f}).**
                * **פקודת לימיט עתידית (Limit Order):** מומלץ למקם פקודת רכש עבור **{res['total_shares_to_buy']}** יחידות ברמת התמיכה ב-**${res['waiting_target']:.2f}**.
                """)
                ...
                st.markdown(f"""
                * **שלב א':** קנה רק **{res['shares_p1']}** יחידות במחיר הנוכחי (**${res['current_price']:.2f}**).
